...
TCGA BRCA Subset:
This script works with TCGA BRCA samples. First, it subsets to all patient samples with HER2 amplifications by using GISTIC data from TCGA. For these patient samples, we look up clinical information to get overall survival (OS) and split the data into patients survivng less than 3 years/ more than 3 years. We use these subsets (Short is <3 YRS; Long is >3 YRS) for prioritizing genes below. For the aims in this project, we are interested in genes amplified or over-expressed in the Short group only, as those are candidate resistance genes that will be tested in our screen.
