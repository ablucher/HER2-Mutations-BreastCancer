...
TCGA BRCA Subset:
This script works with TCGA BRCA samples. First, it subsets to all patient samples with HER2 amplifications by using GISTIC data from TCGA. For these patient samples, we look up clinical information to get overall survival (OS) and split the data into patients survivng less than 3 years/ more than 3 years. We use these subsets (Short is <3 YRS; Long is >3 YRS) for prioritizing genes below. 

Prioritizing co-amplified and co-expressed genes:
Then it pulls out CN amplified genes for these samples and uses a test of proportions to identify genes amplified in the Short survival group. Then for RNASeq data (queried through TCGAbiolinks), we assess differentially expressed genes between the two groups, and focus on those genes that are up-regulated in the Short survival group. 
...
dataDEGs <- TCGAanalyze_DEA(mat1 = BRCAMatrixShort,
                            ...
                            Cond1type = "Short",
                            Cond2type = "Long",
                            fdr.cut = 0.20 , #let's see out to FDR<.20, we likely will use <0.10
                            logFC.cut = 1,
                            method = "glmLRT")
...
#output test
#write_tsv(dataDEGsFiltLevel, here("output", "Table_ShortVsLong_UppQuantNorm_DE_FoldChange_FDR.tsv"))
...
#write_tsv(ShortHits, here("output", "Table_ShortVsLong_UppQuantNorm_DE_FoldChange_FDR_ShortHits.tsv"))
...
#output full
write_tsv(ShortHits_LibraryAnnotation, here("output", "Table_ShortVsLong_UppQuantNorm_DE_FoldChange_FDR_ShortHits_LibraryAnnotation.tsv"))
...
#output
write_tsv(GeneLibrary_Subset, here("output", "Library_SubsetToQueryGenes.tsv"))
